(function(c, l) {
	...
	}) : "undefined" !== typeof exports ? module.exports = l(require("jquery"), c, c.document, c.Math) : l(
		jQuery, c, c.document, c.Math)
	...
	c.fn.fullpage = function(d) {
		function La() {
			...
			c(".fp-section").each(function(a) {
				...
				"undefined" !== typeof d.sectionsColor[a] && b.css("background-color", d.sectionsColor[
					a]);
				"undefined" !==
				...
				typeof d.anchors[a] && (b.attr("data-anchor", d.anchors[a]), b.hasClass("active") && I(d
					.anchors[a], a));
				d.menu && d.css3 && c(d.menu).closest(".fullpage-wrapper").length && c(d.menu).appendTo(
					r);
			...
			a.length && (0 !== c(".fp-section.active").index(".fp-section") || 0 === c(".fp-section.active")
				.index(".fp-section") && 0 !== a.index()) && U(a);
			...
			if (!d.animateAnchor && (a = l.location.hash.replace("#", "").split("/")[0], a.length)) {
				...
				b.length && (d.autoScrolling ? x(b.position().top) : (x(0), t.scrollTop(b.position().top)), I(a,
					null), c.isFunction(d.afterLoad) && d.afterLoad.call(b, a, b.index(".fp-section") +
					1), b.addClass("active").siblings().removeClass("active"))
			}
			ma();
			n.on("load", function() {
		...
		function Qa(a) {
			...
			"#fff" != d.controlArrowColor && (a.find(".fp-controlArrow.fp-next").css("border-color",
				"transparent transparent transparent " + d.controlArrowColor), a.find(
				".fp-controlArrow.fp-prev").css("border-color",
		...
		function Pa() {
			...
			var a = c("#dn-nav");
			a.addClass(function() {
				return d.showActiveTooltip ? "fp-show-active " + d.navigationPosition : d
					.navigationPosition
			});
			...
			c("#dn-nav").find("li").eq(c(".fp-section.active").index(".fp-section")).find("a").addClass(
				"active")
		}
		...
		function oa() {
			...
			if (!d.autoScrolling || d.scrollBar) {
				for (var b = n.scrollTop(), g = 0, f = p.abs(b - m.querySelectorAll(".fp-section")[0]
						.offsetTop), h = m.querySelectorAll(".fp-section"), k = 0; k < h.length; ++k) {
			...
			if (!d.autoScrolling || d.scrollBar) {
				if (!a.hasClass("active") &&
					...
					if (l.length) var r = l.data("anchor"),
						q = l.index();
					y && (a.addClass("active").siblings().removeClass("active"), c.isFunction(d.onLeave) && d
						.onLeave.call(b, g, k, f), c.isFunction(d.afterLoad) && d.afterLoad.call(a, h, k),
						K(a), e.setFitToSection(!a.hasClass("fp-auto-height")), I(h, k - 1), d.anchors
						.length && (A = h, Y(q, r, h, k)));
					clearTimeout(Z);
				...
				d.fitToSection && (clearTimeout(aa), aa = setTimeout(function() {
					y && d.fitToSection && (c(".fp-section.active").is(a) && requestAnimFrame(
						function() {
							v = !0
						}), B(a), v = !1)
				}, d.fitToSectionDelay))
		...
		function pa(a) {
			return a.find(".fp-slides").length ? a.find(".fp-slide.active").find(".fp-scrollable") : a.find(
				".fp-scrollable")
		}
		...
		function L(a, b) {
			if (k.m[a]) {
				...
				if (0 < b.length)
					if (d = "top" === d ? !b.scrollTop() : "bottom" === d ? b.scrollTop() + 1 + b
						.innerHeight() >=
		...
		function Sa(a) {
			...
			if (!qa(a.target) && ba(b)) {
				...
				var g = pa(a);
				y && !z && (b = ra(b), D = b.y, M = b.x, a.find(".fp-slides").length && p.abs(N - M) > p.abs(E -
					D) ? p.abs(N - M) > n.width() / 100 * d.touchSensitivity && (N > M ? k.m.right && e
					.moveSlideRight() : k.m.left && e.moveSlideLeft()) : d.autoScrolling && p.abs(E -
					D) > n.height() / 100 * d.touchSensitivity && (E > D ? L("down", g) : D > E && L(
					"up",
					g)))
			}
		}
		...
		function qa(a, b) {
			...
			var g = c(a).parent();
			return b < d.normalScrollElementTouchThreshold && g.is(d.normalScrollElements) ? !0 : b == d
				.normalScrollElementTouchThreshold ? !1 : qa(g, ++b)
		}
		...
		function B(a, b, g) {
			requestAnimFrame(function() {
				...
				if ("undefined" !== typeof f) {
					var e = a.hasClass("fp-auto-height") ? f.top == 0 ? 0 : f.top - q + a.height() : f
						.top,
						f = {
							element: a,
							callback: b,
					...
					if (!(f.activeSection.is(a) && !v || d.scrollBar && n.scrollTop() === f.dtop && !a
							.hasClass("fp-auto-height"))) {
						if (f.activeSlide.length) var h = f.activeSlide.data("anchor"),
							k = f.activeSlide.index();
						d.autoScrolling && d.continuousVertical && "undefined" !== typeof f
							.isMovementUp && (!f.isMovementUp && "up" == f.yMovement || f
								.isMovementUp && "down" == f.yMovement) && (f.isMovementUp ? c(
									".fp-section.active").before(f.activeSection.nextAll(
									".fp-section")) :
								...
								c(".fp-section.active").after(f.activeSection.prevAll(".fp-section")
									.get().reverse()), x(c(".fp-section.active").position().top), va(),
								f.wrapAroundElements = f.activeSection, f.dest = f.element.position(), f
								.dtop = f.dest.top, f.yMovement = X(f.element));
						if (c.isFunction(d.onLeave) && !f.localIsResizing) {
							if (!1 === d.onLeave.call(f.activeSection, f.leavingSection, f
									.sectionIndex + 1, f.yMovement)) return;
							Ua(f.activeSection)
						}
		...
		function Va(a) {
			if (d.css3 && d.autoScrolling && !d.scrollBar) wa("translate3d(0px, -" + a.dtop + "px, 0px)", !0), d
				.scrollingSpeed ? ca = setTimeout(function() {
					da(a)
		...
		function da(a) {
			a.wrapAroundElements && a.wrapAroundElements.length && (a.isMovementUp ? c(".fp-section:first")
				.before(a.wrapAroundElements) : c(".fp-section:last").after(a.wrapAroundElements), x(c(
					".fp-section.active").position().top), va());
			a.element.find(".fp-scrollable").mouseover();
			e.setFitToSection(!a.element.hasClass("fp-auto-height"));
			c.isFunction(d.afterLoad) && !a.localIsResizing && d.afterLoad.call(a.element, a.anchorLink, a
				.sectionIndex + 1);
			na(a.element);
			y = !0;
		...
		function F(a, b) {
			...
			if (d.onSlideLeave) {
				...
				if (!n && "none" !== t && c.isFunction(d.onSlideLeave) && !1 === d.onSlideLeave.call(r, k, h +
						1, q, t, f)) {
					z = !1;
					return
				}
			...
			n || K(b);
			!d.loopHorizontal && d.controlArrows && (e.find(".fp-controlArrow.fp-prev").toggle(0 !== f), e.find(
				".fp-controlArrow.fp-next").toggle(!b.is(":last-child")));
			...
			d.css3 ? (g = "translate3d(-" + p.round(g.left) + "px, 0px, 0px)", za(a.find(".fp-slidesContainer"),
				0 < d.scrollingSpeed).css(Aa(g)), fa = setTimeout(function() {
				u()
		...
		function I(a, b) {
			d.menu && (c(d.menu).find(".active").removeClass("active"), c(d.menu).find('[data-menuanchor="' +
				a + '"]').addClass("active"));
			d.navigation && (c("#dn-nav").find(".active").removeClass("active"), a ? c("#dn-nav").find(
					'a[href="#' + a + '"]').addClass("active") : c("#dn-nav").find("li").eq(b).find("a")
				.addClass("active"))
		}
		...
		function J(a) {
			...
			var b = a.closest(".fp-section"),
				...
				f;
			c.length ? f = c.get(0).scrollHeight : (f = a.get(0).scrollHeight, d.verticalCentered && (f = a
				.find(".fp-tableCell").get(0).scrollHeight));
			b = q - parseInt(b.css("padding-bottom")) - parseInt(b.css("padding-top"));
			f > b ? c.length ? c.css("height", b + "px").parent().css("height", b + "px") : (d
				.verticalCentered ? a.find(".fp-tableCell").wrapInner('<div class="fp-scrollable" />') : a
		...
		function Da(a) {
			var b = q;
			if (d.paddingTop || d.paddingBottom) b = a, b.hasClass("fp-section") || (b = a.closest(
					".fp-section")), a = parseInt(b.css("padding-top")) +
		...
		function Y(a, b, c, f) {
			...
			d.anchors.length && !d.lockAnchors && (a ? ("undefined" !== typeof c && (f = c),
				...
				"undefined" === typeof b && (b = a), ea = b, Ga(f + "/" + b)) : ("undefined" !==
				typeof a && (ea = b), Ga(c)));
			ma()
		}
		...
		function Ma() {
			...
			for (var d in c) a.style[d] !== H && (a.style[d] = "translate3d(1px,1px,1px)", b = l
				.getComputedStyle(a).getPropertyValue(c[d]));
		...
		(function() {
			...
			d.scrollBar && d.scrollOverflow && T("warn",
			...
			c.each(d.anchors, function(a, b) {
				(c("#" + b).length || c('[name="' + b + '"]').length) && T("error",
		...
		e.setMouseWheelScrolling = function(a) {
			a ? m.addEventListener ? (m.addEventListener("mousewheel", u, !1), m.addEventListener("wheel",
					u, !1), m.addEventListener("DOMMouseScroll", u, !1)) : m.attachEvent("onmousewheel",
					u) : m.addEventListener ? (m.removeEventListener("mousewheel", u, !1), m
					.removeEventListener("wheel", u, !1), m.removeEventListener("DOMMouseScroll", u, !1)) :
		...
		e.reBuild = function(a) {
			if (!h.hasClass("fp-destroyed")) {
				...
				c(".fp-section").each(function() {
					...
					d.verticalCentered && c(this).find(".fp-tableCell").css("height", Da(c(this)) +
						"px");
		...
		e.setResponsive = function(a) {
			...
			a ? b || (e.setAutoScrolling(!1, "internal"), e.setFitToSection(!1, "internal"), c("#dn-nav")
				.hide(), h.addClass("fp-responsive")) : b && (e.setAutoScrolling(G.autoScrolling,
					"internal"), e.setFitToSection(G.autoScrolling, "internal"), c("#dn-nav").show(), h
				.removeClass("fp-responsive"))
		};
		var z = !1,
		...
		w.keydown(function(a) {
			...
			b.is("textarea") ||
				...
				b.is("input") || b.is("select") || !d.keyboardScrolling || !d.autoScrolling || (-1 < c
					.inArray(a.which, [40, 38, 32, 33, 34]) && a.preventDefault(), O = a.ctrlKey, Ka =
					setTimeout(function() {
							var b = a.shiftKey;
							switch (a.which) {
								case 38:
		...
		c(".fp-section").on("click touchstart", ".fp-controlArrow", function() {
			c(this).hasClass("fp-prev") ? k.m.left && e.moveSlideLeft() : k.m.right && e
				.moveSlideRight()
		});
		n.resize(Ba);
		var ga = q;
		e.destroy = function(a) {
			...
			w.off("click", "#dn-nav a").off("mouseenter", "#dn-nav li").off("mouseleave", "#dn-nav li").off(
				"click", ".fp-slidesNav a").off("mouseover", d.normalScrollElements).off("mouseout", d
